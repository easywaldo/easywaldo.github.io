...
> JPQL Query 중 @Id 대상이 아닌 컬럼으로 Fetch 되는 메서드는 영속화 컨텍스트를 거치지 않고 직접 쿼리 실행을 한다.



### 직접 실행을 사용하면 이득이 되는 경우

<U>JPQL Natvie SQL 또는 EntityManager 를 Refresh 하여 영속화 모델을 새로고침 한 후
Query 들을 다시 Fetch 하게 되는 경우 DB에 있는 최신 데이터를 조회할 수 있다.</U>

이렇게 최신 데이터 확인이 우선시 되는 경우엔 해당 전략을 사용하는 것이 이점이 있으며
그렇지 않고 한번 영속화된 데이터가 세션수준에서 관리가 되어도 무방하다면 해당 데이터를 EntityManager 영속화 수준에서 이용하도록 하는 것이 이점이 있겠다.
